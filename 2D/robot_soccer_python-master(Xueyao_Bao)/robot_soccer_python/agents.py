...
class Environment:
    ...
    def draw_players_and_ball(self):
        ...
        pygame.draw.circle(self.window, GRAY_COLOR, (center[0], center[1]), self.list_radius[0], 2)  # ✅ 描边
    ...
    def draw_field(self):
        ...
        border_px = int(BORDER_M * M2PIX)
        pitch_w = int(PITCH_LENGTH_M * M2PIX)
        pitch_h = int(PITCH_WIDTH_M * M2PIX)

        x0, y0 = border_px, border_px
        x1, y1 = x0 + pitch_w, y0 + pitch_h

        midx = (x0 + x1) // 2
        midy = (y0 + y1) // 2
        center_r = int((CENTER_CIRCLE_D_M / 2) * M2PIX)

        white = (255, 255, 255)

        # Outer pitch rectangle
        pygame.draw.rect(self.window, white, Rect(x0, y0, pitch_w, pitch_h), 3)

        # Halfway line
        pygame.draw.line(self.window, white, (midx, y0), (midx, y1), 3)

        # Center circle
        pygame.draw.circle(self.window, white, (midx, midy), center_r, 3)
    ...
    def draw_soccer_goal_and_scoreboard(self):
        ...
        goal_w = int(GOAL_WIDTH_M * M2PIX)
        goal_d = int(GOAL_DEPTH_M * M2PIX)

        goal_y = midy - goal_w // 2

        # Left goal (behind goal line x0)
        pygame.draw.rect(self.window, (0, 0, 0),
                         Rect(x0 - goal_d, goal_y, goal_d, goal_w), 0)

        # Right goal (behind goal line x1)
        pygame.draw.rect(self.window, (0, 0, 0),
                         Rect(x1, goal_y, goal_d, goal_w), 0)
