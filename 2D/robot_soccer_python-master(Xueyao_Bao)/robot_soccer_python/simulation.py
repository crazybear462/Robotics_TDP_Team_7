...
class Simulation:
    ...
    def check_collision_between_ball_players(self):
        ...
        for players in self.player:
            ...
            if dist_player <= (RADIUS_BALL + players.radius):
    ...
    def check_goal(self):
        ...
        """
        Change for TDP purpose
        """

        ball_x = self.ball.pose.position.x  # meters
        ball_y = self.ball.pose.position.y  # meters
        ball_r = self.ball.radius  # meters (Ball.radius)

        # Pitch geometry in meters
        x0 = BORDER_M
        y0 = BORDER_M
        x1 = BORDER_M + PITCH_LENGTH_M
        y1 = BORDER_M + PITCH_WIDTH_M

        midy = (y0 + y1) / 2.0
        goal_half = GOAL_WIDTH_M / 2.0

        in_goal_y = (midy - goal_half) <= ball_y <= (midy + goal_half)

        # Ball crosses left goal line -> RIGHT team scores (keep original scoreboard meaning)
        if (ball_x - ball_r) <= x0 and in_goal_y:
            if (datetime.datetime.now() - self.goal).seconds > 1:
                self.right_goal += 1
                self.goal = datetime.datetime.now()

        # Ball crosses right goal line -> LEFT team scores
        if (ball_x + ball_r) >= x1 and in_goal_y:
            if (datetime.datetime.now() - self.goal).seconds > 1:
                self.left_goal += 1
                self.goal = datetime.datetime.now()
    ...
    def draw(self, window, environment):
        ...
        for i in range(len(self.player)):
            list_centers.append(np.array(
                [round(M2PIX * self.player[i].pose.position.x), round(M2PIX * self.player[i].pose.position.y)]))
