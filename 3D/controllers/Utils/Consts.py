...
# Constants
TIME_STEP: int = 40  # Simulation time step in milliseconds
PI: float = math.pi  # Pi constant for angular calculations

# Initial translation positions for all entities on the pitch (x, y, z coordinates in meters)
INITIAL_TRANSLATIONS: Dict[str, List[float]] = {
...
# Initial rotation quaternions for all entities (axis-angle representation: x, y, z, angle)
INITIAL_ROTATIONS: Dict[str, List[float]] = {
    "BALL": [0, 1, 0, 0.0],              
    "RED_GK": [0, 1, 0, 0.1],           
    "RED_DEF_L": [0, 1, 0, 0.1],       
    "RED_DEF_R": [0, 1, 0, 0.1],          
...
# Ball positions for out-of-bounds scenarios (x, y, z coordinates)
BALL_POSITIONS: Dict[str, List[float]] = {
    "OUT_R": [-3.65, 0, 0.0798759],       # Ball out on red side
    "OUT_B": [3.65, 0, 0.0798759]         # Ball out on blue side
}

class Motions:
    """Initialize all motion objects with their corresponding motion files."""
    def __init__(self): 
...
class MotionBase(Motion):
    def __init__(self, name: str, path: str):
        super().__init__(path)  # Call parent Motion class constructor with file path
        self.name = name        # Store the motion name for reference and debugging