...
def _field_bounds_from_play_zone():
  """
  Derive pitch bounds from RedTeamStrategies.PLAY_ZONE to avoid hardcoding.
  PLAY_ZONE entries look like: {zone_id: [(x0,y0), (x1,y1)], ...}
  """
  xs = []
  ys = []
  for _, corners in RedTeamStrategies.PLAY_ZONE.items():
...
class Forward (SoccerRobot):
  ...
  def decideMotion(self, ballCoordinate, selfCoordinate):
    ...
    goal_mid = RedTeamStrategies.BLUE_GOAL["Middle"]
